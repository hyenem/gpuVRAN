...
| 장 | 내용 |
|----|------|
| **① GPU 아키텍처** | CPU vs GPU, SM 내부 구조, SIMT·warp, 메모리 계층, occupancy |
| **② CUDA 프로그래밍** | grid/block/thread 모델, 첫 커널, 메모리 관리, 동기화 |
| **③ 실행 모델** | 커널 launch의 실체, 비동기·스트림, 이벤트, 블록→SM 배정 |
| **④ 스케줄링** | warp 스케줄링, occupancy 튜닝, 스트림 우선순위, 선점, MIG·MPS |
| **⑤ CUDA Graphs** | 3단계 동작, stream capture, 업데이트, conditional node, device launch |
| **⑥ 성능 최적화** | 메모리 병합, shared memory 타일링, bank conflict, 프로파일링 |
